"""
network_interface.py (Bollinger Node)

This module manages the communication between the Bollinger trading node and external systems. It serves as an API server for the node, handles transactions, and fetches performance data.
...
3. **Data Synchronisation**:
   - Fetches performance metrics from peer nodes (e.g., the SMA Node) for comparative analysis.
...
Handlers:
- `handle_get_performance_data`: Returns the performance data for the Bollinger node.
...
This module ensures smooth communication between the Bollinger node, the ledger manager, and other nodes in the blockchain trading system.
"""
...
class NetworkInterface:
    def __init__(self, wallet, security, performance_manager):
        ...
        self.other_node_url = "http://<SMA_NODE_IP:PORT>/get_performance_data"
        ...
        try:
            ...
            async with session.get(self.other_node_url) as resp:
                if resp.status == 200:
                    ...
                    print(f"Failed to fetch performance data from SMA Node: {resp.status}")
                    return None
        except Exception as e:
    ...
    async def handle_test_endpoint(self, request):
        return web.Response(text="Bollinger Node Interface Running.")
