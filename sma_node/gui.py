...
This module provides a graphical user interface (GUI) for interacting with the SMA trading node. It allows users to monitor and control the trading process while visualising key metrics and trade history.
...
3. **Node Comparison**:
   - Fetches performance data from other nodes (e.g., Bollinger Node) to display comparative metrics.
...
This module acts as a user-friendly interface for real-time monitoring and control of the SMA trading strategy, ensuring users can oversee trading activity and performance metrics effectively.
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SMA Trading Node")
        self.setGeometry(100, 100, 800, 600)

        self.security = Security()
        self.wallet = Wallet(self.security)
        self.performance_manager = PerformanceManager(self.wallet, node_name="SMA")
        ...
        self.other_node_url = "http://<BOLLINGER_IP:PORT>/get_performance_data"  # Bollinger node perf endpoint
    ...
    def initUI(self):
        ...
        # Welcome label
        self.welcome_label = QLabel("Welcome to the SMA Trading Node!")
        ...
        self.compare_label = QLabel("Comparison with Bollinger Node:")
    ...
    async def fetch_other_node_data(self):
        async with aiohttp.ClientSession() as session:
            try:
                ...
                self.compare_text.setText("Unable to fetch Bollinger node data.")

    def update_comparison_display(self, other_data):
        my_data = self.performance_manager.get_performance_data()
        text = "Comparing SMA to Bollinger Node:\n\n"
        text += f"SMA ROI: {my_data['roi_since_start']*100:.2f}% vs Bollinger ROI: {other_data['roi_since_start']*100:.2f}%\n"
        text += f"SMA Trades: {my_data['trades_executed']} vs Bollinger Trades: {other_data['trades_executed']}\n"
        text += f"SMA P/L: ${my_data['total_profit_loss']:.2f} vs Bollinger P/L: ${other_data['total_profit_loss']:.2f}\n"
    ...
    def closeEvent(self, event):
        ...
        if clicked_button == cancel_button:
            ...
            perf_file = os.path.join(current_dir, "performance_sma.json")
